...
contract CourseRegistration is Ownable {
    ...
    event PaymentMade(address indexed user, string email, uint256 amount);
    ...
    function payForCourse(string memory email) public payable {
        require(msg.value == courseFee, "Payment must be able to course fee.");
                //msg.value is in Wei, 1 ETH = 10^18 wei
        payments.push(Payment(msg.sender, email, msg.value));
        emit PaymentMade(msg.sender, email, msg.value);
    }
    ...
    function getPaymentsByUser(address _user) public view returns (Payment[] memory) {
        ...
        for(uint i = 0; i < payments.length; i++) {
            if(payments[i].user == _user) count++;
        }
